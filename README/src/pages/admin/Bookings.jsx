...
const Bookings = () => {
  ...
  const parseComplaints = (text) => {
    if (!text) return { interval: 'N/A', complaint: 'N/A' };
    if (text.startsWith('Service Interval:')) {
      const match = text.match(/Service Interval:\s*(.*?)\s*\((.*)\)/);
      if (match) {
        return { interval: match[1].trim(), complaint: match[2].trim() };
      }
      const splitIdx = text.indexOf('(');
      if (splitIdx !== -1) {
         return {
            interval: text.substring(17, splitIdx).trim(),
            complaint: text.substring(splitIdx).replace(/^\(|\)$/g, '').trim()
         };
      }
      return { interval: text.replace('Service Interval:', '').trim(), complaint: 'N/A' };
    }
    return { interval: 'N/A', complaint: text };
  };
        ...
        <table className="premium-table">
          <thead>
            <tr>
              ...
              <th className="premium-th">Interval</th>
          ...
          <tbody>
            {bookings.length === 0 ? (
              ...
              bookings.map((booking) => {
                const { interval, complaint } = parseComplaints(booking.complaints);
                return (
                <tr key={booking.id} className="premium-tr">
                  ...
                  <td className="premium-td text-slate-500 dark:text-slate-400">
                    {interval}
                  </td>
                  <td className="premium-td min-w-[300px]">
                    <span className="text-slate-600 dark:text-slate-300 block whitespace-normal leading-relaxed">{complaint}</span>
                  </td>
                  <td className="premium-td">
